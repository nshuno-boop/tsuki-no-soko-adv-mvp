...
- 檜山徹は序盤の完成度を上げるため、生命維持主任らしい作業服、疲労、誠実さが読める立ち絵へ再生成した。
- 北条リツは澪と被らないよう、短いアッシュ系の髪、細い目元、眼鏡、前傾した技術者姿勢、ケーブルプローブと腕端末を持つAI管理技師として再生成した。
...
- 会話ウィンドウ、名前欄、選択肢に制作済みUI画像を接続した。Player View QA後は、左端の見切れリスクを直し、会話窓左側へ話者の顔アップを追加した。名前欄はセリフ欄内に収め、浮いた印象を抑えた。セリフ欄内の白い括弧状装飾と破線は本文に干渉しないよう削除し、外枠中心のデザインへ整理した。
...
- 会話画面: テキストボックスと名前欄を細線・シアン発光のUIに再接続した。名前欄と本文をセリフ欄内へ収め、左側に顔アップを出すことで、本文と表情を同時に追いやすくした。本文安全領域には白い括弧装飾を置かない。
...
- Phase 6.5では、タイトルロゴPNGに日本語タイトルを直接入れ直し、調査ハブの固定パネルをコンパクト化して1280x720下端のはみ出し感を軽減した。ノアは短い乱れ髪、フーディ、月星モチーフの若者デザインとして再生成した。
- Phase 6.6では、標準メインメニューが起動経路で飛ばされる場合に備えて、`splashscreen` で専用タイトル画面を表示してから本編へ入る形にした。ジンは企業広報法務として、スーツ、ID、法務タブレット、橙アクセントの立ち絵へ更新した。
