...
Player View QA後、目的表示と立ち絵の重なりを解消するため、立ち絵配置を腰上構図寄りに調整しました。あわせて、澪をセナと同じ腰上寄りの構図で再生成し、セナは澪の色替えに見えないよう顔立ち・髪型・衣装シルエットを再生成しました。会話窓左側には話者の顔アップを追加し、主要背景も追加でブラッシュアップしています。

- 背景9点を1920x1080の月面都市ビジュアルへ更新
- 主要立ち絵差分とALMA差分を透明PNGとして更新
- 序盤の印象強化として檜山徹の立ち絵差分を本素材寄りに更新
- 第2章以降の見栄え強化として、北条リツを澪と被らないAI管理技師デザインへ更新
...
- タイトルロゴPNGに日本語タイトルを入れ直し、調査ハブの固定UIをコンパクト化して下端のはみ出し感を軽減
- ノアを月生まれの若者らしい短髪・フーディ・月星モチーフのデザインで再生成
- 起動経路によって標準メインメニューが呼ばれない場合でも、`splashscreen` でタイトル画面を必ず表示してから本編へ入るよう調整
- ジンを企業広報法務らしいスーツ、ID、法務タブレット、橙アクセントのデザインで再生成
